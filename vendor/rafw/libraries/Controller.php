...
abstract class Ra_Controller
{
	...
	public final function __construct($route = null)
	{
		...
		//load helpers
		foreach (glob(RA_HELPERS_PATH . "/*.php") as $helper) {
			require_once $helper;
		}
		
		foreach (glob(RA_SLICES_PATH . "/*/app/helpers/*.php") as $helper) {
			require_once $helper;
		}
	...
	public static function build_view_path($controller, $action)
	{
		...
		$files = glob(RA_SLICES_PATH . '/*/app/views' . $sufix);
		$files[] = RA_VIEWS_PATH . $sufix;
	...
	{
		$layouts = glob(RA_SLICES_PATH . "/*/app/views/layouts/" . $layout . '.php');
		$layouts[] = RA_VIEWS_PATH . '/layouts/' . $layout . '.php';
	...
	{
		return $this->base_url(RA_INDEX_PAGE . $sulfix);
	}
	...
	public function cache_block_end()
	{
		...
		$cache_path = RA_CACHE_PATH . "/{$identifier}.block";
		...
		$tmp_path = tempnam(RA_TMP_PATH, "BC");
	...
	public function url_to($params)
	{
		...
		if (!preg_match("/^[a-z]+:\/\//", $route)) {
			$route = RA_BASE_URL . RA_INDEX_PAGE . $route;
		}
	...
	public function stylesheet_tag($css, $attr = array())
	{
		...
		$real_path = RA_PUBLIC_PATH . '/' . $path_sufix;
