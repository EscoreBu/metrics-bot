...
class Ra_FormHelper
{
	...
	public static function text_field($field, $options = array(), $html_options = array())
	{
		...
		if (!is_a($object, 'BlankObject') && $object->field_has_errors($field)) {
			$attributes['class'] .= self::class_error;
		}
	...
	public static function relational_field($field, $options = array(), $html_options = array())
	{
		...
		$out .= "<br /><br />";
		$out .= self::build_tag('button', array_merge(array('onclick' => "Ra.Util.move_options('#{$source_id}', '#{$destiny_id}')"), $button_html), 'Adicionar &darr;');
		$out .= self::build_tag('button', array_merge(array('onclick' => "Ra.Util.move_options('#{$destiny_id}', '#{$source_id}')"), $button_html), 'Remover &uarr;');
		$out .= "<br /><br />";
		$out .= self::select_for_model($field . '_destiny', $object->$normal, $options['value_field'], $options['id_field'], array(), $common_html);
		$out .= "<script type=\"text/javascript\"> Ra.Util.relational_map('#{$destiny_id}', '#{$hidden_id}') </script>";
	...
	public static function checkbox_tree($field, $roots, $options = array(), $html_options = array())
	{
		...
		if(isset($html_options['class_div_inside']))
			$class_div_inside = $html_options['class_div_inside'];
		else
			$class_div_inside = 'span6';
		...
		for($i = 1; $i <= $columns; $i++) $content_columns[$i] = "";
		...
		foreach ($roots as $item)
		{
			...
			$content_columns[$count_column] .= self::build_tag('label', array('class' => 'checkbox'), $tag);
		...
		for($i = 1; $i <= $columns; $i++) $content_columns[$i] = self::build_tag('div', array('class' => $class_div_inside), $content_columns[$i]);
		return implode(" ", $content_columns);
	}
	...
	private static function build_tag($tagname, $attributes = array(), $content = false, $force_open = false)
	{
		...
		if(@$attributes['jq'])
		{
			if($attributes['jq'] == 'currency')
			{
				$pos_content = "<script>\$(function(){\$('#$attributes[id]').maskMoney({thousands:'', decimal:'.', allowZero:true, defaultZero:true})})</script>";
			}
			else if($attributes['jq'] == 'mask')
			{
				$pos_content = "<script>\$(function(){\$('#$attributes[id]').mask(\"$attributes[jq_value]\");})</script>";
			}
		}
		...
		if ($force_open) {
			return "<$tagname $attributes>$pos_content";
		}
		
		if ($content !== false) {
			return "<$tagname $attributes>$content</$tagname>$pos_content";
		} else {
			return "<$tagname $attributes />$pos_content";
		}
	}
