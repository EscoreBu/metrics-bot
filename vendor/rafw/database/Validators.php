...
class ActiveRecord_Validators
{
	public static $validations_messages = array(
		...
		'invalid_size_of' => 'The field %s minimum of $s letter',
		'invalid_match_post_of' => 'The field not match',
	...
	public static function validates_size_of($object, $field, $value, $err)
	{
		// de(array($object, $field, $value, $options));
		$size = strlen($object->$field);
		
		if ($size < $value) {
			$object->add_error($field, $err ? $err : sprintf(self::$validations_messages['invalid_size_of'], $field, $value));
			return false;
		}
